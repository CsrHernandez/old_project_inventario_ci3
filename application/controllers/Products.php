...
class Products extends CI_Controller {
    ...
    public function create($id = -1) {
        ...
        $this->form_validation->set_rules('description', 'Se requiere una descripcion', 'required');

        if ($this->form_validation->run() === FALSE) {

            $session_data = $this->session->userdata('logged_in');//Agregado
            $data['username'] = $session_data['username'];//
            $data['realname'] = $session_data['realname'];//
            ...
            $session_data = $this->session->userdata('logged_in');//Agrgado
            $data['username'] = $session_data['username'];//
            $data['realname'] = $session_data['realname'];//
